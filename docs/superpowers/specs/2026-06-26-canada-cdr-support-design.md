...
- `apps/dsa-web`: account **create** exposes the dropdown (when ca) + grouped selector (`<optgroup>` by type, untyped last). **Web account-edit UI + web custom-value input are deferred** (user-confirmed 2026-06-27) — a follow-up work item; the web has no account-edit flow today, and `account_type` is fully editable via the API meanwhile. No tax logic.
