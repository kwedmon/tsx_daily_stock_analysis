...
- `apps/dsa-web`: account **create** exposes the dropdown (when ca) + grouped selector (`<optgroup>` by type, untyped last) + card display. **Web account-edit UI and web custom-value input are deferred** — the web has no account-edit flow (`updateAccount()` does not exist); backend update is fully editable via API. No tax logic.
