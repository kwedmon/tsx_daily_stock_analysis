...
**Account type:**
- `PortfolioAccount.account_type` — new nullable `String`; idempotent `ALTER TABLE` migration + `schema_migrations` bump.
- New module `src/portfolio/account_types.py` with the option set; Canadian set when `market=ca`: `rrsp, tfsa, fhsa, rrif, resp, lira, rdsp, non_registered_cash, non_registered_margin` (extensible). Advisory validation (free value allowed) to avoid breaking other markets.
- API schema + `apps/dsa-web`: account create/edit exposes `account_type` (dropdown when ca). UI groups **accounts** by `account_type` (one type per account). No tax logic.
